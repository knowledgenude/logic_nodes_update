...
class AddRigidBodyNode extends LogicNode {
	...
	public var property0: String; // Collision shape
	...
	override function run(from: Int) {
		...
		if ((group < 0) || (group > 32)) group = 1; // Limit max groups to 32

		if ((mask < 0) || (mask > 32)) mask = 1; // Limit max masks to 32

		if (rb == null) {
			// TODO: Use lower case there to minimize typos ?
			switch(property0) {
				case "Box": shape = Box;
